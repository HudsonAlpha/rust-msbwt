...
pub fn convert_to_vec(bwt: impl Read) -> Vec<u8> {
    ...
    for c in bwt.bytes() {
        ...
        if ch == curr {
            ...
            while count > 0 {
                let write_byte: u8 =
                    translate[curr as usize] | ((count as u8 & COUNT_MASK) << LETTER_BITS) as u8;
                ret.push(write_byte);
                count >>= NUMBER_BITS;
            }
            curr = ch;
...
pub fn save_bwt_numpy(bwt: impl Read, filename: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let header_string =
        b"\x93NUMPY\x01\x00\x56\x00{\'descr\': \'|u1\', \'fortran_order\': False, \'shape\': (";
...
/// ```
pub fn save_bwt_runs_numpy(
    runs: impl Iterator<Item = (u8, u64)>,
    filename: &str,
...
mod tests {
    ...
    fn test_save_bwt_numpy() {
        ...
        let file: NamedTempFile = Builder::new()
            .prefix("temp_data_")
            .suffix(".npy")
            .tempfile()
            .unwrap();
