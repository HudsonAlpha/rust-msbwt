...
fn decode_run(run: u16) -> (u8, u16) {
    ((run & SYMBOL_MASK) as u8, (run >> SYMBOL_BITS))
}
...
impl RLEBlock {
    ...
    unsafe fn increment_run(&mut self, run_index: usize) {
        ...
        let overflow = self
            .runs
            .get_unchecked(run_index)
            .overflowing_add(SINGLE_COUNT);
        *self.runs.get_unchecked_mut(run_index) = overflow.0;
        if overflow.1 {
            ...
            *self.runs.get_unchecked_mut(run_index) |= HALF_FULL;
            self.runs
                .insert(run_index + 1, *self.runs.get_unchecked(run_index));
        }
    }
    ...
    pub fn split(&mut self) -> RLEBlock {
        ...
        let new_runs_data: ArrayVec<u16, CAPACITY_BUFFER> = self
            .runs
            .drain(breakpoint..)
            .map(|v| {
...
mod tests {
    ...
    fn test_block_splits() {
        ...
        runs.try_extend_from_slice(&vec![encode_run(0, 256)])
            .unwrap();
        assert_eq!(
            block,
            RLEBlock {
        ...
        let mut runs = ArrayVec::<u16, CAPACITY_BUFFER>::new();
        runs.try_extend_from_slice(&vec![encode_run(1, 1), encode_run(0, 256)])
            .unwrap();
        assert_eq!(
            new_block,
            RLEBlock {
        ...
        let mut runs = ArrayVec::<u16, CAPACITY_BUFFER>::new();
        runs.try_extend_from_slice(&vec![encode_run(1, 1), encode_run(0, 255)])
            .unwrap();
        assert_eq!(
            new_block,
            RLEBlock {
        ...
        let new_block: RLEBlock = block.split();
        let mut runs = ArrayVec::<u16, CAPACITY_BUFFER>::new();
        runs.try_extend_from_slice(&vec![encode_run(0, 255)])
            .unwrap();
        assert_eq!(
            block,
            RLEBlock {
        ...
        let mut runs = ArrayVec::<u16, CAPACITY_BUFFER>::new();
        runs.try_extend_from_slice(&vec![encode_run(1, 1), encode_run(0, 1)])
            .unwrap();
        assert_eq!(
            new_block,
            RLEBlock {
