...
fn encode_run(run: &mut [u8], symbol: u8, count: u64) -> usize {
    ...
    //1
    if count < 64 {
        //1 bit for symbol, 3 for control, so 4 for length
        run[0] = symbol | (count << SYMBOL_BITS) as u8;
        1
    } else if count < 1024 {
        //1 bit for symbol, 3+2 for control, so 16-6 = 10 for length
        run[0] = 0xC0 | symbol | (count >> 6 << SYMBOL_BITS) as u8;
        run[1] = 0x80 | (count & 0x3F) as u8;
        2
    }
    else if count < 4194304 {
        //1 bit for symbol, 3+2+2+2 for control, so 32-10 = 22 for length
        run[0] = 0xE0 | symbol | (count >> 18 << SYMBOL_BITS) as u8;
        run[1] = 0x80 | ((count >> 12) & 0x3F) as u8;
        run[2] = 0x80 | ((count >> 6) & 0x3F) as u8;
        run[3] = 0x80 | (count & 0x3F) as u8;
        4
    }
    else if count == 4194304 {
        println!("split");
        //1 bit for symbol, 3+2+2 for control, so 24-8 = 16 for length
        run[0] = 0xE0 | symbol | (2097152 >> 18 << SYMBOL_BITS) as u8;
        run[1] = 0x80 | ((2097152 >> 12) & 0x3F) as u8;
        run[2] = 0x80 | ((2097152 >> 6) & 0x3F) as u8;
        run[3] = 0x80 | (2097152 & 0x3F) as u8;
        run[4] = run[0];
        run[5] = run[1];
        run[6] = run[2];
        run[7] = run[3];
...
fn decode_run(run: &[u8]) -> (u8, u64, usize) {
    if run[0] & 0x80 == 0 {
        ...
        (
            run[0] & SYMBOL_MASK, 
            (run[0] >> SYMBOL_BITS) as u64, 
            1
        )
    }
    else if run[0] & 0xE0 == 0xC0 {
        //2 bytes
        (
            run[0] & SYMBOL_MASK,
            (((run[0] & 0x1E) as u64) << 5) | ((run[1] & 0x3F) as u64),
            2
        )
    } else {
        //3 bytes
        (
            run[0] & SYMBOL_MASK,
            (((run[0] & 0x1E) as u64) << 17) | (((run[1] & 0x3F) as u64) << 12) | (((run[2] & 0x3F) as u64) << 6) | (run[3] & 0x3F) as u64,
            4
        )
    }
}
...
mod tests {
    ...
    fn test_to_vec() {
        ...
        for i in 0..1024 {
            ...
            assert_eq!(block.to_vec(), vec![1; i+1]);
        }
